...
class PublicationsService(object):
    ...
    def insert_publication(self, publication):
        try:
            if (self.publications_repository.create_publication(publication)): return True
            else: return False

        except Exception as e:
            return False
        
    def get_all_publication(self, usrId):
        ans = list()
        try:
            cursor = self.publications_repository.publications_by_id(usrId)
            print(cursor)
            for publication in cursor:
                ans.append(publication)
                print(publication)
            return ans
